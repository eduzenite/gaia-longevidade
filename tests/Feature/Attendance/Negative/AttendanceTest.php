...
class AttendanceTest extends TestCase
{
    ...
    public function update_attendance_with_wrong_fields()
    {
        ...
        $this->put(route('attendance.update', ['id' => $Attendance->id]), $data)
            ->assertStatus(400)
            ->assertJson(['error' => 'Bad Request']);
    }
    ...
    public function update_non_existent_attendance()
    {
        ...
        $this->put(route('attendance.update', ['id' => 0]), $data)
            ->assertStatus(404)
    ...
    {
        $this->withoutExceptionHandling();
        $this->get(route('attendance.show', ['id' => 0]))
            ->assertStatus(404)
            ->assertJson(['error' => 'Not Found']);
    }
    ...
    {
        $this->withoutExceptionHandling();
        $this->delete(route('attendance.destroy', ['id' =>0]))
            ->assertStatus(404)
            ->assertJson(['error' => 'Not Found']);
    }
}
